...
pro.nombre AS producto, v.nombre AS nombre_vendedor, v.apellido AS apellido_vendedor, SUM(pro.precio) AS total
FROM clientes c 
INNER JOIN pedidos p, productos pro, pedidos_has_productos pp, vendedores v 
WHERE 
c.id=p.id_clientes AND p.id=pp.pedidos_id AND pro.id=pp.productos_id AND v.id = p.id_vendedores AND pro.id=8
GROUP BY p.id;

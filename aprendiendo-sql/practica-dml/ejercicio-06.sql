...
SELECT m.id AS id_mesonero, m.nombre AS nombre, m.apellido AS apellido, COUNT(p.id) AS pedidos, salario*2 AS salario
FROM mesonero m INNER JOIN pedidos p  ON m.id=p.id_mesonero
WHERE p.fecha LIKE '%-09-%'
GROUP BY m.id HAVING COUNT(p.id)>=2;

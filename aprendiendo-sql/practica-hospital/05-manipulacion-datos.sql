 ...
 */
 ...
 AND p.id_ocupacion=o.id AND p.id=1;
...
WHERE c.id_paciente=p.id AND c.id_medico=m.id
ORDER BY c.fecha;
...
a.nombre AS archivo, a.ruta AS ruta
 ...
 AND ce.id_estudio=e.id AND p.id=1;
 ...
 */
 ...
 SELECT CONCAT(m.nombre,' ',m.apellido) AS nombre_medico,
 n.nacionalidad AS nacionalidad, m.email AS correo
 FROM medicos m INNER JOIN nacionalidad n
 ON n.id=m.id_nacionalidad WHERE m.id=1;

 -- Especialidades

 SELECT CONCAT(m.nombre,' ',m.apellido) AS nombre_medico,
 e.especialidad AS especialidad FROM medicos m INNER JOIN
 especialidad e, medico_has_especialidad me
 WHERE m.id=me.id_medico AND e.id=me.id_especialidad AND m.id=1;

 -- Hospitales en los cuales trabaja
 SELECT CONCAT(m.nombre,' ',m.apellido) AS nombre_medico,
 h.nombre AS hospital FROM medicos m INNER JOIN medico_has_hospital mh, hospital h
 WHERE m.id=mh.id_medico AND mh.id_hospital=h.id AND m.id=1;

 -- Pacientes atendidos
 SELECT CONCAT(m.nombre,' ',m.apellido) AS nombre_medico,
 CONCAT(p.nombre,' ',p.apellido) AS nombre_paciente,
 c.motivo AS motivo, c.fecha AS fecha FROM medicos m 
 INNER JOIN pacientes p, consultas c
 WHERE m.id=c.id_medico AND p.id=c.id_paciente AND m.id=1;
...
CREATE VIEW PACIENTES_AGENDADOS AS 
	SELECT
	    a.fecha_final AS comienza,
	    a.fecha_final AS finaliza,
	    CONCAT(p.nombre, ' ', p.apellido) AS nombre_paciente,
	    p.email AS email_paciente,
	    CONCAT(m.nombre, ' ', m.apellido) AS nombre_medico
	FROM pacientes p
	    INNER JOIN agenda a, medicos m
	WHERE
	    p.id = a.id_paciente
	    AND m.id = a.i
ID_MEDICO; 
...
CREATE VIEW CONSULTAS_ESTUDIOS AS 
	SELECT
	    ...
	    c.motivo AS motivo,
	    e.nombre AS estudio,
	    ce.valor AS resultados,
	    CONCAT(p.nombre, ' ', p.apellido) AS nombre_paciente,
	    CONCAT(m.nombre, ' ', m.apellido) AS nombre_medico
	FROM pacientes p
	    INNER JOIN estudios e, consultas c, consulta_has_estudios ce, medicos m
	WHERE
	    m.id = c.id_medico
	    AND p.id = c.id_paciente
	    AND c.id = ce.id_consulta
	    AND e.id = ce.i
ID_ESTUDIO; 